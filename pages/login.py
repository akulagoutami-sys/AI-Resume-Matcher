...
<h2 class="auth-title" style="font-size: 2.2rem; background: -webkit-linear-gradient(45deg, #00d4ff, #c084fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 5px;">Welcome Back</h2>
<p style="color: #94a3b8; letter-spacing: 1px; margin-top: 0;">Access your intelligent dashboard</p>
</div>
""", unsafe_allow_html=True)
        ...
        if st.button("🚀 Secure Log In", type="primary", use_container_width=True):
            if login_user(email, password):
                st.success("Authentication successful! Initializing AI engine...")
                st.rerun()
            else:
                st.error("Invalid credentials. Neural link denied.")
